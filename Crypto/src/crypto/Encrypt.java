package crypto;

import java.util.Random;
import static crypto.Helper.*;

public class Encrypt {
	
	public static final int CAESAR = 0;
	public static final int VIGENERE = 1;
	public static final int XOR = 2;
	public static final int ONETIME = 3;
	public static final int CBC = 4; 
	
	public static final byte SPACE = 32;
	
	final static Random rand = new Random();
	
	//-----------------------General-------------------------
	
	/**
	 * General method to encode a message using a key, you can choose the method you want to use to encode.
	 * @param message the message to encode already cleaned
	 * @param key the key used to encode
	 * @param type the method used to encode : 0 = Caesar, 1 = Vigenere, 2 = XOR, 3 = One time pad, 4 = CBC
	 * 
	 * @return an encoded String
	 * if the method is called with an unknown type of algorithm, it returns the original message
	 */
	public static String encrypt(String message, String key, int type) {
		
		//plainText
		
		int lengthPlainTextString = message.length();
		byte[] plainTextByte = new byte[lengthPlainTextString];
		
		plainTextByte = stringToBytes(message);

		//key
		
		byte[] keyByte = new byte[key.length()];
		
		keyByte = stringToBytes(key);
		
		//type
		
		byte[] cipherTextByte = new byte[lengthPlainTextString];
		
		if(CAESAR==type)
		{
			cipherTextByte = caesar(plainTextByte, keyByte[0]);
		}
		else if(VIGENERE==type)
		{
			cipherTextByte = vigenere(plainTextByte, keyByte);
		}
		else if(XOR==type)
		{
			cipherTextByte = xor(plainTextByte, keyByte[0]);
		}
		else if(ONETIME==type)
		{
			cipherTextByte = oneTimePad(plainTextByte, keyByte);
		}
		else if(CBC==type)
		{
			cipherTextByte = cbc(plainTextByte, keyByte);
		}
		else
		{
			return message;
		}

		//conversion
		
		String cipherTextString = bytesToString(cipherTextByte);
		
		return cipherTextString; 
	}
	
	
	//-----------------------Caesar-------------------------
	
	/**
	 * Method to encode a byte array message using a single character key
	 * the key is simply added to each byte of the original message
	 * @param plainText The byte array representing the string to encode
	 * @param key the byte corresponding to the char we use to shift
	 * @param spaceEncoding if false, then spaces are not encoded
	 * @return an encoded byte array
	 */
	public static byte[] caesar(byte[] plainText, byte key, boolean spaceEncoding) {
		assert(plainText != null);
		
		int lengthPlainText = plainText.length;
		byte[] cipherText = new byte[lengthPlainText];
		
		if(false==spaceEncoding)
		{	
			for(int byteIndex=0; byteIndex<lengthPlainText; byteIndex++)
			{
				if(SPACE == plainText[byteIndex])
				{
					cipherText[byteIndex] = plainText[byteIndex];
				}
				else
				{
					cipherText[byteIndex] = (byte) (plainText[byteIndex] + key);
				}
			}
		}
		return cipherText;
	}
	
	/**
	 * Method to encode a byte array message  using a single character key
	 * the key is simply added  to each byte of the original message
	 * spaces are not encoded
	 * @param plainText The byte array representing the string to encode
	 * @param key the byte corresponding to the char we use to shift
	 * @return an encoded byte array
	 */
	public static byte[] caesar(byte[] plainText, byte key) {

		return caesar(plainText, key, false); 
	}
		
	//-----------------------XOR-------------------------
	
	/**
	 * Method to encode a byte array using a XOR with a single byte long key
	 * @param plaintext the byte array representing the string to encode
	 * @param key the byte we will use to XOR
	 * @param spaceEncoding if false, then spaces are not encoded
	 * @return an encoded byte array
	 */
	public static byte[] xor(byte[] plainText, byte key, boolean spaceEncoding) {
		
		int lengthPlainText = plainText.length;
		byte[] cipherText = new byte[lengthPlainText];
		
		if(false==spaceEncoding)
		{
			for(int byteIndex=0; byteIndex<lengthPlainText; byteIndex++)
			{
				if(SPACE == plainText[byteIndex])
				{
					cipherText[byteIndex] = plainText[byteIndex];
				}
				else
				{
					cipherText[byteIndex] = (byte) (plainText[byteIndex] ^ key);
				}
			}
		}
		
		return cipherText;
	}
	/**
	 * Method to encode a byte array using a XOR with a single byte long key
	 * spaces are not encoded
	 * @param key the byte we will use to XOR
	 * @return an encoded byte array
	 */
	public static byte[] xor(byte[] plainText, byte key) {
		
		return xor(plainText, key, false); 
	}
	
	//-----------------------Vigenere-------------------------
	
	/**
	 * Method to encode a byte array using a byte array keyword
	 * The keyword is repeated along the message to encode
	 * The bytes of the keyword are added to those of the message to encode
	 * @param plainText the byte array representing the message to encode
	 * @param keyword the byte array representing the key used to perform the shift
	 * @param spaceEncoding if false, then spaces are not encoded
	 * @return an encoded byte array 
	 */
	public static byte[] vigenere(byte[] plainText, byte[] keyword, boolean spaceEncoding) {
	
		int lengthKeyword = keyword.length;
		int lengthPlainText = plainText.length;
		byte[] cipherText = new byte[lengthPlainText];
		int key = 0;
		int outphase = 0; //counts every time a space bar is encountered 
		
		if(false==spaceEncoding)
		{
			for(int byteIndex=0; byteIndex<lengthPlainText; byteIndex++)
			{
				key = keyword[ (byteIndex-outphase)%lengthKeyword ];
				
				if(SPACE == plainText[byteIndex])
				{
					cipherText[byteIndex] = plainText[byteIndex];
					outphase++;
				}
				else
				{
					cipherText[byteIndex] = (byte) (plainText[byteIndex] + key);
				}	
			}
		}
		return cipherText;
	}
	
	/**
	 * Method to encode a byte array using a byte array keyword
	 * The keyword is repeated along the message to encode
	 * spaces are not encoded
	 * The bytes of the keyword are added to those of the message to encode
	 * @param plainText the byte array representing the message to encode
	 * @param keyword the byte array representing the key used to perform the shift
	 * @return an encoded byte array 
	 */
	public static byte[] vigenere(byte[] plainText, byte[] keyword) {
		
		return vigenere(plainText, keyword, false);
	}
	
	
	
	//-----------------------One Time Pad-------------------------
	
	/**
	 * Method to encode a byte array using a one time pad of the same length.
	 *  The method  XOR them together.
	 * @param plainText the byte array representing the string to encode
	 * @param pad the one time pad
	 * @return an encoded byte array
	 */
	public static byte[] oneTimePad(byte[] plainText, byte[] pad) {
		
		int lengthPad = pad.length;
		int lengthPlainText = plainText.length;
		
		/*Pad length -> must not be shorter than text*/
		assert(lengthPad >= lengthPlainText);	

		byte[] cipherText = new byte[lengthPlainText]; 	
		
		for(int byteIndex=0; byteIndex<lengthPlainText; byteIndex++)
		{
			cipherText[byteIndex] = (byte) (plainText[byteIndex] ^ pad[byteIndex]);
		}
		return cipherText;
	}
	
	
	
	
	//-----------------------Basic CBC-------------------------
	
	/**
	 * Method applying a basic chain block counter of XOR without encryption method. Encodes spaces.
	 * @param plainText the byte array representing the string to encode
	 * @param iv the pad of size BLOCKSIZE we use to start the chain encoding
	 * @return an encoded byte array
	 */
	public static byte[] cbc(byte[] plainText, byte[] iv) {
		
		int lengthIv = iv.length;
		int lengthPlainText = plainText.length;
		int newByteIndex = 0;
		
		byte[] cipherText = new byte[lengthPlainText]; 	
		
		//We do the oneTimePad algorithm for all the elements of iv
		
		for(int byteIndex=0; byteIndex<lengthPlainText && byteIndex<lengthIv; byteIndex++)
		{
			cipherText[byteIndex] = (byte) (plainText[byteIndex] ^ iv[byteIndex]);
		}
		
		//If lengthIv>=lengthPlainText then the cipherText is already done
		
		if(lengthIv>=lengthPlainText)
		{
			return cipherText;
		}
		
		//else with a smart index we re-use the elements already in cipherText for the cbc algorithm
		
		for(int byteIndex=lengthIv; byteIndex<lengthPlainText; byteIndex++)
		{
			newByteIndex = byteIndex-lengthIv ;
			cipherText[byteIndex] = (byte) (plainText[byteIndex] ^ cipherText[newByteIndex]);
		}
		return cipherText;
	}
	
	
	/**
	 * Generate a random pad/IV of bytes to be used for encoding
	 * @param size the size of the pad
	 * @return random bytes in an array
	 */
	public static byte[] generatePad(int size) {
		
		byte[] pad = new byte[size];
		
		for(int indexByte=0; indexByte<size; indexByte++)
		{
			pad[indexByte] = (byte) (rand.nextInt(255));
		}

		return pad;
	}
	
	//-----------------------BONUS PCBC-------------------------
	//https://en.wikipedia.org/wiki/Block_cipher_mode_of_operation  
	//Propagating_cipher_block_chaining_(PCBC)
	
	
	/**
	 * Method applying a basic chain block counter of XOR with encryption method at each step. Encodes spaces.
	 * @param plainText the byte array representing the string to encode
	 * @param iv the pad of size BLOCKSIZE we use to start the chain encoding
	 * @return an encoded byte array
	 */
	public static byte[] pcbc(byte[] plainText, byte[] iv) {
		
		int lengthIv = iv.length;
		int lengthPlainText = plainText.length;
		int newByteIndex = 0;
		
		byte[] cipherText = new byte[lengthPlainText]; 	
		
		//We do the oneTimePad algorithm for all the elements of iv
		
		for(int byteIndex=0; byteIndex<lengthPlainText && byteIndex<lengthIv; byteIndex++)
		{
			cipherText[byteIndex] = (byte) (plainText[byteIndex] ^ iv[byteIndex]);
		}
		
		//If lengthIv>=lengthPlainText then the cipherText is already done
		
		if(lengthIv>=lengthPlainText)
		{
			return cipherText;
		}
		
		//else with a smart index we re-use the elements already in cipherText for the pcbc algorithm
		
		for(int byteIndex=lengthIv; byteIndex<lengthPlainText; byteIndex++)
		{
			newByteIndex = byteIndex-lengthIv ;
			cipherText[byteIndex] = (byte) (plainText[byteIndex] ^ (cipherText[newByteIndex] ^ plainText[newByteIndex]));
		}
		return cipherText;
	}
	
}
